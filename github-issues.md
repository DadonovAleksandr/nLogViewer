...
**Tasks:**
- [ ] Convert `ReadLogFile()` method to async
- [ ] Implement `GetAllAsync()` and `GetNewAsync()` methods
- [ ] Use `IAsyncEnumerable<string>` for streaming file content
- [ ] Add `CancellationToken` support
- [ ] Update `ILogReader` interface to include async methods
...
**Tasks:**
- [ ] Dispose `_reader` field in `LogViewer.Dispose()`
- [ ] Stop timer before disposal
- [ ] Implement disposal pattern in `FileLogReader`
- [ ] Add event handler cleanup in `LogViewerViewModel`
- [ ] Unsubscribe from `_filter.RefreshFilter` event
...
**Tasks:**
- [ ] Wrap `Process()` method content in try-catch
